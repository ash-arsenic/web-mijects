...
const paras = document.querySelector("#paragraph");
const text_show = document.querySelector(".text");

form.addEventListener("submit", function (e) {
  e.preventDefault();
  const value = parseInt(paras.value);
  let paragraphs = "";
  console.log(value);
  if (isNaN(value) || value < 1 || value > 10) {
    paragraphs = getParagraph(1);
  } else {
    paragraphs = getParagraph(value);
  }
  text_show.innerHTML = paragraphs;
});

function getParagraph(num) {
  let paragaph = "";
  for (let i = 0; i < num; i++) {
    const random = Math.floor(Math.random() * text.length);
    paragaph += text[random];
    paragaph += "<br><br>";
  }
  return paragaph;
}
